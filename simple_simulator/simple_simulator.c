...
#define RAND 7 			// "000111"; -- RAND Rx / store random number in Rx register
...
int main()
{
	...
	int PC=0, IR=0, SP=0, MAR=0, rx=0, ry=0, rz=0, regRand=0, IncRand=0, COND=0, RW=0, DATA_OUT=0;
...
loop:
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				case RAND:
					...
					if (pega_pedaco(regRand, 15, 15)==1)
						regRand = TAMANHO_MEMORIA-30; // quando regRand==TAMANHO_MEMORIA, reseta p/ inicio
	...
	else if (selM1 == sRand) M1 = regRand;

	//if (selM1==sRand) printf("MEMORY[%d] -> %d\n", M1, MEMORY[M1]);
